...
export class ColorWheel extends Component {
  ...
  componentDidMount = () => {
    const {h, s, v} = colorsys.hex2Hsv(this.props.initialColor);
    this.setState({v});
  ...
  resetPanHandler () {
    if (!this.state.panHandlerReady) return;
  ...
  updateColor = ({nativeEvent}) => {
    ...
    const hsv = {h: deg, s: 100 * radius, v: this.state.v};
  ...
  animatedUpdate = color => {
    ...
    this.props.onColorChange({h, s, v: this.state.v});
          ...
          <View style={{flex: 1}}>
            <Slider
                ...
                onValueChange={(value) => {
                  ...
                  const hsv = {h, s, v: value};
                  const currentColor = colorsys.hsv2Hex(hsv);
                  this.setState({hsv, currentColor, v: value});
                  this.props.onColorChange(hsv);
                }}
            />
          </View>
